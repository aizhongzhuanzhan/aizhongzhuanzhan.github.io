...
function limitPayload(payload) {
  ...
  const sites = [...payload.sites]
    ...
    .filter((site) => {
      const key = safeUrl(site.url).toLowerCase();
...
function shuffleSites(sites, updatedDate) {
  const result = [];
  for (let start = 0; start < sites.length; start += SHUFFLE_BAND) {
    const band = sites.slice(start, start + SHUFFLE_BAND)
      .map((site) => ({ site, key: seededNumber(`${updatedDate}:${start}:${site.sourceRank}:${site.url}`) }))
      .sort((a, b) => a.key - b.key || a.site.sourceRank - b.site.sourceRank)
      .map(({ site }) => site);
    result.push(...band);
  }
...
async function syncData() {
  const response = await fetch(SOURCE_URL, {
    headers: { "user-agent": "aizhongzhuanzhan-static-builder/1.0" },
  ...
  const snapshot = limitPayload(incoming);
...
<html lang="zh-CN">
  ...
  <body>
    ...
    <header class="topbar">
      <a class="wordmark" href="../" aria-label="AI 中转站推荐首页"><span>AI 中转站</span><strong>推荐</strong></a>
...
function renderStructuredData({ page, canonical, title, description, sites, totalSites, updatedDate }) {
  ...
  graph.push({
    ...
    name: page === 1 ? SITE_NAME : `${SITE_NAME}第 ${page} 页`,
...
function renderPage({ page, totalPages, sites, allSites, updatedDate }) {
  ...
  const title = page === 1
    ? `AI 中转站推荐｜${allSites.length} 家 API 中转站排名、对比与选择指南`
    : `AI 中转站推荐第 ${page} 页｜排名 ${first}–${last}`;
  const description = page === 1
    ? `AI 中转站推荐收录 ${allSites.length} 家 API 中转站，静态对比在线率、响应延迟、模型覆盖、支付方式与服务政策，并提供计费、安全和接入测试指南。`
  ...
  const hero = page === 1
    ? `<p class="eyebrow">AI API SERVICE INDEX · ${updatedDate.replaceAll("-", ".")}</p>
          <h1>AI 中转站推荐<br /><em>排名与选择指南</em></h1>
...
<html lang="zh-CN">
  ...
  <body>
    <a class="skip-link" href="#main">跳到主要内容</a>
    <header class="topbar">
      <a class="wordmark" href="${root}/" aria-label="AI 中转站推荐首页"><span>AI 中转站</span><strong>推荐</strong></a>
    ...
    <main id="main">
      ...
      <section class="ranking" id="ranking" aria-labelledby="ranking-title">
        <div class="ranking-head">
          <div><p>RANKING / ${String(page).padStart(2, "0")}</p><h2 id="ranking-title">AI 中转站推荐列表</h2></div>
          <p>当前显示第 ${first}–${last} 名，共 ${allSites.length} 家</p>
        </div>
        <p class="ranking-note">指标会随线路和服务状态变化；相近候选会按数据日期在每 5 名范围内轮换展示。建议结合自己的网络、模型与调用方式小额测试，不要仅凭展示次序、单次测速或宣传价格决定长期使用。</p>
...
async function build() {
  ...
  const normalized = payload.sites.map(normalizeSite).sort((a, b) => a.sourceRank - b.sourceRank);
  const sites = shuffleSites(normalized, updatedDate);
  const snapshot = { ...payload, sites };
  await atomicWrite(DATA_PATH, `${JSON.stringify(snapshot, null, 2)}\n`);
  ...
  process.stdout.write(`已生成 ${totalPages} 个分页、${topicPages.length} 个模型专题，共 ${sites.length} 个站点（上限 ${MAX_SITES}）；数据日期 ${updatedDate}\n`);
}
